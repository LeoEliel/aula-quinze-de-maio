...
# BANCO DE DADOS
def x_get_db__mutmut_3():
    # Abre e retorna uma conexão com o banco SQLite cujo caminho está
    # em app.config['DATABASE']. O valor vem de config.py.
    conn = sqlite3.connect(app.config["XXDATABASEXX"])
    conn.row_factory = sqlite3.Row
    return conn


# BANCO DE DADOS
def x_get_db__mutmut_4():
    # Abre e retorna uma conexão com o banco SQLite cujo caminho está
    # em app.config['DATABASE']. O valor vem de config.py.
    conn = sqlite3.connect(app.config["database"])
    conn.row_factory = sqlite3.Row
...
def init_db():
    args = []# type: ignore
    kwargs = {}# type: ignore
    return _mutmut_trampoline(x_init_db__mutmut_orig, x_init_db__mutmut_mutants, args, kwargs, None)
...
def x_init_db__mutmut_2():
    # Cria as tabelas e insere os dados iniciais se ainda não existirem.
    conn = get_db()
    conn.executescript(None)
    conn.commit()
    conn.close()

x_init_db__mutmut_mutants : ClassVar[MutantDict] = { # type: ignore
